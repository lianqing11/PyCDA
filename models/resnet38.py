...
class ResBlock(nn.Module):
    def __init__(self, in_channels, mid_channels, out_channels, stride=1, first_dilation=None, dilation=1):
        ...
        self.bn_branch2a = SynchronizedBatchNorm2d(in_channels)

        self.conv_branch2a = nn.Conv2d(in_channels, mid_channels, 3, stride,
                                       padding=first_dilation, dilation=first_dilation, bias=False)

        self.bn_branch2b1 = SynchronizedBatchNorm2d(mid_channels)
...
class ResBlock_bot(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, dilation=1, dropout=0.):
        ...
        self.bn_branch2b1 = SynchronizedBatchNorm2d(out_channels//4)
        self.dropout_2b1 = torch.nn.Dropout2d(dropout)
        self.conv_branch2b1 = nn.Conv2d(out_channels//4, out_channels//2, 3, padding=dilation, dilation=dilation, bias=False)

        self.bn_branch2b2 = SynchronizedBatchNorm2d(out_channels//2)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.bn7 = SynchronizedBatchNorm2d(4096)

        self.not_training = [self.conv1a]

        self.normalize = Normalize()
        self.conv6a = nn.Conv2d(4096, 512, 3, padding=12, dilation=12)
        self.linear19 = nn.Conv2d(512, 19, 3, padding=12, dilation=12)
    ...
    def forward(self, x):
        ...
        conv7 = F.relu(self.conv6a(conv6))
        conv7 = self.linear19(conv7)
...
def convert_mxnet_to_torch(filename):
    ...
    for k, v in save_dict.items():
        ...
        elif 'conv6a' in toks[0]:
            if 'weight' in toks[1]:
                renamed_dict['conv6a.weight'] = v
            elif 'bias' in toks[1]:
                renamed_dict['conv6a.bias'] = v
        elif 'linear19' in toks[0]:
            if 'weight' in toks[1]:
                renamed_dict['linear19.weight'] = v
            elif 'bias' in toks[1]:
                renamed_dict['linear19.bias'] = v
...
def resnet38(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(
            convert_mxnet_to_torch("/data/lianqing/.torch/models/gta_rna-a1_cls19_s8_ep-0000.params"), strict=False,
      )
    return model
